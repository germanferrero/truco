...
class TrucoViewTests(TestCase):
    ...
    """
    def test_view_crear_partida(self):
    ...
    """
    def test_view_unirse_partida(self):
    ...
    """
    Test que testea la vista partida1
    """
    def test_view_partida(self):
        lobby = Lobby()
        user1 = User.objects.get(username='test_user1')
        user2 = User.objects.get(username='test_user2')
        partida = Partida.objects.get(nombre='Partida1')
        lobby.unirse_partida(user2, partida)
        response = self.client.get(reverse(('truco:partida'), args = (partida.id,)))
        self.assertEqual(response.status_code, 302)
